...
class get_games(Resource):
    ...
    def get(self):
        ...
        games_dict = [g.get_detail() for g in games]
        return {'data': games_dict}, 200


join_game_parser = api.parser()
join_game_parser.add_argument(
    'SeatID',
    type=int,
    location='form',
    help='The seat id.')
...
class join_game(Resource):
    ...
    def post(self, game_guid, seat_id, **kwargs):
        data = join_game_parser.parse_args
        user = kwargs['user']
        stack = data['Stack']
        seat_id = data['SeatID']
        game = Game.fetch(game_guid)
        player = game.join(user, stack, seat_id)
        if not player:
            return {'error': 'Internal Server Error'}, 500
        return {'message': 'Successfully joined game'}, 200